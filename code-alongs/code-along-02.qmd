...
This chunk will not run (on purpose!) when rendered because `eval: false`.

**Install the [`summarytools`](https://cran.r-project.org/web/packages/summarytools/vignettes/introduction.html) package, available on CRAN.** **Click the green arrow icon to run the following code chunk.**
...
[Operators in R](https://www.geeksforgeeks.org/r-language/r-operators/) are symbols directing R to perform various kinds of mathematical, logical, and decision operations.
...
You can use [`as_factor`](https://haven.tidyverse.org/reference/as_factor.html) to see the value labels of the variable `premarsx`.
...
Use these options with the `freq()` function for the variable `premarsx`.
