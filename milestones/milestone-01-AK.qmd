...
*Use the `gss_all` dataset for checkpoints 01 - 04.*

*Use the `gss24` dataset for checkpoints 05 - 10.*

## Checkpoint 01

***Report the survey question that corresponds with the `fefam`
variable. How did you find out?***

`?fefam`

Survey question: Now I'm going to read several more statements. As I
read each one, please tell me whether you strongly agree, agree,
...
`fefam` is an ordinal variable because the response categories (e.g.,
Strongly Agree, Agree, etc.) have a meaningful order but there are not
equal intervals (such as numeric values).
...
1977, 1985-1986, 1988 - 1991, 1993-1994, 1996-2018 (every other year),
2021-2022, 2024
...
The respondent `strongly disagreed` with the statement "It is much
...
gss24$fefam <- zap_missing(gss24$fefam) 
gss24$fefam <- as_factor(gss24$fefam) 
gss24$fefam <- droplevels(gss24$fefam) 
```
...
freq(gss24$fefam, report.nas = FALSE, headings = FALSE)
...
167 respondents `strongly agree` with the statement. 30.48% of
...
Most respondents (57.67%) were `pretty happy` in 2024.
...
Women (21.3%) were slightly more likely than men (20.2%) to say they
were `very happy`.

## Checkpoint 10

***According to the table above, were a larger percentage of women or
men `not too happy` in 2024?***

A slightly larger percentage of men (22.7%) than women (20.4%) were
`not too happy` in 2024.
...
- Finally, create an IPUMS account for the dataset of interest. See this
  [help video](https://www.youtube.com/watch?v=Oo7RYMV7q4Y) for details.
