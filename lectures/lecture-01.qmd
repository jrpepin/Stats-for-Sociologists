...
::: notes
-   open R script or Quarto document (Use code-along-02 as an example)  
...
script]{style="color: #3498DB"}" or "[Quarto
Document]{style="color: #3498DB"}."
...
## Install `gssr()` and `gssrdoc()` {.sparse-slide}
...
##  {data-menu-title="`gss_which_years()`"}

We can find which years one or more variables were asked with the
`gss_which_years()` function.
