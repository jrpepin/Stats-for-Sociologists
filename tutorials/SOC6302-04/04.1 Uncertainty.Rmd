...
![](images/snail04.png "I have confidence in sunshine....I have confidence in ME!"){width="75%"}
...
1. Wheelan, Charles. 2014. “Inference: Why My Statistics Professor Thought I Might Have Cheated.” Pp. 143–56 in *Naked Statistics: Stripping the Dread from the Data*. New York: W. W. Norton & Company.
2. Wheelan, Charles. 2014. “Polling: How We Know That 64 Percent of Americans Support the Death Penalty.” Pp. 169–79 in *Naked Statistics: Stripping the Dread from the Data*. New York: W. W. Norton & Company.
...
Social scientists are typically interested in comparing outcomes between groups. 

This time, we'll compare confidence intervals instead of the only the mean and median. 
We can use add variables based on our equations using `summarize()`. 

Let's compare of television hours for men and women.  

1. Use the `group_by()` function to look at differences by `sex`.
2. Click "Run Code" to see your results.  
...
tvhrs <- gss24 |>
  ...
  summarise(
    ...
    sd_tv = sd(tvhours),
    se_tv = sd_tv / sqrt(n),
    lowCI = mean_tv - (1.96 * se_tv),
    hiCI = mean_tv + (1.96 * se_tv)
    )
...
<div id="ci-hint">
**Hint:** Did you replace the blank line with the variable `sex`?
</div>
...
Looking only at the mean and median, we might conclude that on average, men spend more hours watching tv than women because the mean and median for men was greater than the mean and median hours for women. Is this still the case if we compare the confidence intervals?

There is a .95 probability that men's average tv hours per day is between 3.00 and 3.42.
There is a .95 probability that women's average tv hours per day is between 3.19 and 3.57.

We’re 95% confident that women watch tv between 3.19 and 3.57 hours per day. But we have no idea if the true average is closer to 3.19, or 3.57, or somewhere near the middle of our range.

The confidence interval shows much overlap, so we can’t know for sure if there is a difference between the average time men and women spend watching tv per day.
...
tvhrs |>
  ggplot(aes(x = mean_tv, y = sex, color = sex)) +
  geom_point(size = 3) +
  geom_errorbar(aes(xmin = lowCI, xmax = hiCI), width = 0.2, size = 1.2) +
  theme(legend.position = "none") +
  xlim(0, 4) +
  labs(title = "Average hours of tv use per day",
       subtitle = "with 95% confidence intervals",
       y = " ",
       x = " ",
       caption = "US General Social Survey 2024") 
```
  
The mean for men is greater than for women, you protest. This is true, and our best guess of a population parameter is a sample statistic. But, we use confidence intervals to convey our uncertainty. If we drew another random sample of men and women, the means would be slightly different. The confidence intervals convey this uncertainty, whereas the sample statistic makes it appear as if the sample mean is exactly equivalent to what the true population mean must be.  
...
To find the *t* statistic, we combine the formula for the Z score with the formula for the standard error. The Z score is used to identify the place of a raw score on the distribution of responses. 
