...
Use `mutate()` and `case_when()` to recode the `panel` variable so 1a =
2016, 1b = 2018, and 2 = 2020.
...
-   select the variables `yearid`, all `marital` variables, all `wwwhr`
    variables, and all `realrinc` variables
-   pivot the `marital`, `wwwhr`, and `realrinc` variables longer (while
    keeping `yearid`)
...
-   create a summary dataframe that contains the averages for `wwwhr`
    and `realrinc`
...
*NOTE: after you've created a `think_full` dataframe with the
appropriate variables, delete the `echo` and `eval` statements in each
of the code blocks below to produce the necessary tables to answer the
research question.*
