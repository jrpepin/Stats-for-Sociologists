...
library(ggeffects) # ggplots with predictions
...
So far we have examined bivariate linear regressions, an equation model with one independent variable used to predict another variable. But, often we want to use two or more variables to predict the value of the dependent variable. Using more than one independent variable in our equation is referred to as **multiple regression**.  

We have been using the following equation:
...
Often times, we believe believe many predictors can help explain changes our outcome variable. We can adjust our equation by adding another predictor variable to it.  
...
![](images/planes.png "Picture of regression planes with 2 IVs"){ width=100% }
...
This equation is more realistic and better suited for modeling actual data. Unobservable factors are influences on the outcome that we can’t directly measure. But the good news is, some of these factors are observable. Observable factors are all the independent variables we can measure and put into our equation.  
  
### Example  

We might suspect that daily tvhours are related to both someone's age and educational attainment. We could look at how `age` and `educ` both relate to our outcome of `tvhours`. Let's start by looking at a scatterplot of each of these correlations.
...
This yields separate slope estimates for each independent variable
Bivariate slope estimates implicitly assume that the two independent variables are separately related to the outcome.
...
The slopes ($b$) of each IV are sometimes called a regression coefficient, or simply a coefficient. 
A coefficient is just the number used to multiply each variable. In this example, we could say the coefficient for the variable `age` is 0.01. The coefficient for the variable `educ` is -.08.  
...
Let's use the function `get_regression_table()` from the [`moderndive`](https://moderndive.github.io/moderndive/) package.
...
The p-value for each independent variable tests the null hypothesis, that the variable has no correlation with the dependent variable. The p-value for the `age` coefficient (0.014) was0.008 (i.e., p < .001). Therefore, using an alpha level of .05, we can reject the null hypothesis that there is no association between the daily tvhours and age.  

Likewise, the p-value for the `educ` coefficient was .003, so we can say there is a statistically significant negative association between years of education and tvhours.  
...
::: {style="color: #F39C12; font-family: 'Shadows Into Light'"}
...
For example, if we thought the relationship between average TV hours and years of education was conditional on gender, that is the correlation between tv hours and education was different for men and women, we could run two separate regressions.
...
In this equation, the first coefficient, $b_1$, is our continuous variable `educ` that contains respondents years of education. The second coefficient, $b_2$ is a new variable `female` where men are coded as `0` and women are coded as `1`.  


[**What if the case is a man?**]{style="color: #3498DB"} 
...
[**What if the case is a woman?**]{style="color: #3498DB"} 
...
Let's try it out with our tvhours regression example. 
...
The intercept, 3.65, represents the average daily TV hours for men (the reference group) with an education level of zero. 

The coefficient for education (-0.0771), means that for each additional year of education, daily TV hours decrease by about 0.08 hours. 
...
model <- lm(tvhours ~ age + sex, data = gss24)
...
question("Is gender a statistically significant predictor of daily tv hours, after controlling for years of education?",
    answer("No, because the p-value for sex: female is 0.451 and the confidence interval includes zero", correct = TRUE, message = "Nice! This is a test of whether the coefficient is different from 0. "),
    answer("Yes, because the coefficient for sex: female is 0.114 and the p-value is 0.021"),
    answer("Yes, because the p-value for sex: female is 0.000 and the confidence interval does not include zero"),
...
It often helps to visualize the results from a regression model. Let's use the `ggpredict()` function from {ggeffects} package to do so. `ggpredict()` calculates predicted values from a regression model for specific values of the variables. It shows you what your model says the outcome would be if you held some variables constant and changed others.
...
preds <- ggpredict(model, terms = c("educ", "sex"))
...
What about the next row which shows men with 3 years of education: 3.41 average TV hours. We add our intercept of $3.65$ to our `educ` coefficient $-0.0771$ three times (3 units of change):

$3.65 + (-.0771*3)$
$3.65 -0.2313 = 3.419$

And what about the following row showing the predicted average TV hours for women with 3 years of education? The same thing except you also include the `sex` coefficient:

$3.65 + (-.0771*3) + 0.1564$
$3.65 -0.2313 + 0.1564 = 3.575$
...
The interaction term `educ:sexfemale` tells us whether the relationship between education and daily TV hours differs between men and women.
...
The interaction term `educ:sexfemale` is +0.08, which adjusts the slope of education for women. So for women, the effect of education is:
